...
int8_t BME680_configuation( void ){
    ...
    if( rslt != BME680_OK ){
        e_printf("BME680 set sensor settings ERROR\r\n");
    }
 ...
 int8_t BME680_Init( void ){
    ...
    if( rslt != BME680_OK ){
        e_printf("BME680 Init ERROR\r\n");
        return rslt;
    }
    
    e_printf("BME680 init success!\r\n");
...
int8_t BME680_read( int16_t * temprature, uint32_t * pressure, uint32_t * humidity, uint32_t * resistance){
    ...
    if( rslt != BME680_OK ){
        e_printf("BME680 set sensor mode ERROR\r\n");
    }
    ...
    if( rslt != BME680_OK ){
        e_printf("BME680 get sensor data ERROR\r\n");
    }
    ...
    *humidity = data.humidity;
    ...
    e_printf("T: %.2f degC, P: %.2f hPa, H %.2f %%rH ", data.temperature / 100.0f,
